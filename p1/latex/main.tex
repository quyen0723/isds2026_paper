...
This paper makes three contributions: (1) the first controlled cross-platform experimental design for studying prompt--response dynamics in a Southeast Asian educational setting; (2) a proposed five-dimensional AIOQ-R rubric with a planned validation protocol (CVI $\geq 0.80$, $\kappa \geq 0.70$); and (3) an integrated theoretical framework combining the CLEAR framework, Cognitive Load Theory~\cite{Sweller1988}, and TAM2~\cite{Davis1989,Venkatesh2000} to explain the prompt--response mechanism.
...
\begin{itemize}
  ...
  \item \textbf{Gap 2:} Research is exclusively concentrated in Western contexts; no controlled study has been conducted in Southeast Asia, despite comparable adoption rates~\cite{Bui2025}.
...
\textbf{Moderator:} AI platform---ChatGPT (GPT-4o, fixed API version) vs. Claude Sonnet (fixed API version). Temperature is set to 0.7 for both platforms to eliminate systematic variance from sampling behavior.
...
\end{axis}
\end{tikzpicture}
\caption{Anticipated composite AIOQ-R scores by prompt level and platform. Projections are anchored by literature: Sawalha et al.~\cite{Sawalha2024} report $d = 0.69$--$0.94$ for reformulation over copy-paste, and Kumar et al.~\cite{Kumar2025} document a 33\% gain for structured prompting. Error bars represent pre-registration uncertainty bounds; empirical values will replace these projections after Phase~2 data collection.}
\label{fig:expected}
\end{figure}
...
First, it proposes the first cross-platform controlled experimental design for studying prompt--response dynamics in a Southeast Asian educational context, addressing the geographic and methodological gaps in the existing literature. The within-subjects design with Latin Square counterbalancing and fixed API parameters ensures rigorous control over confounding variables.
