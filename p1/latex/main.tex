...
To guarantee reproducibility and eliminate researcher-to-researcher wording drift, each of the three CLEAR levels is mechanically instantiated through the eight-field RCIFENI-O template: \textbf{R}ole, \textbf{C}ontext, \textbf{I}nstructions, \textbf{F}ormat, \textbf{E}xample (optional), \textbf{N}otices, \textbf{I}nput, and \textbf{O}KR. \emph{Level} is therefore a structural-completeness variable rather than a free-form verbal descriptor: the L1 prompt carries only the Input field, L2 adds Role and Instructions, and L3 activates the full mandatory set plus Notices and an explicit objective with key results. For the code-producing problem-solving framing, L3 additionally embeds a Gherkin Given/When/Then acceptance contract so that the accept criterion is behaviourally explicit. Figure~\ref{fig:rcifeni-o} summarises the CLEAR~$\to$~RCIFENI-O gating; the full verbatim templates accompany the paper as supplementary material.
...
\foreach \y/\lbl in {-1/Role, -2/Context, -3/Instructions, -4/Format,
                     -5/Example, -6/Notices, -7/Input, -8/OKR}
...
\node[act] at (4.6, -3.6){Notices};
\node[act] at (4.6, -4.2){Input};
\node[act] at (4.6, -4.8){OKR};

% Gherkin annotation under L3 (problem-solving only)
\node[actG, minimum width=2.1cm, minimum height=0.55cm] at (4.6, -5.6) {+Gherkin};
\node[font=\scriptsize, anchor=north, align=center] at (4.6, -5.95)
  {\itshape problem-solving only};
...
A computational pilot verified the end-to-end pipeline before the IRB-gated Phase~2. We sampled $N=\PilotN$ problems with a fixed seed from HumanEval~\cite{Chen2021HumanEval} (164 Python function-completion tasks) and produced \PilotSessions{} sessions through GPT-4o-mini (three CLEAR-grounded prompt levels $\times$ three pedagogical framings: algorithm explanation, complexity analysis, and problem solving). Prompt levels were instantiated through the RCIFENI-O framework (see Appendix), so \emph{level} is a structural-completeness variable rather than a free-form verbal directive. An automatic GPT-4o-mini judge scored each response on the five AIOQ-R dimensions on a 1--5 Likert scale, producing a composite in the 5--25 range. A Friedman test across the three prompt levels, treating HumanEval problems as repeated blocks, yielded $\chi^2(\PilotFriedmanDF)=\PilotFriedmanChi$, $p=\PilotFriedmanP$, Kendall's $W=\PilotKendallW$.
...
Per-cell composite means (basic / intermediate / advanced) were \PilotAlgoBasic{} / \PilotAlgoInt{} / \PilotAlgoAdv{} for algorithm explanation, \PilotCxBasic{} / \PilotCxInt{} / \PilotCxAdv{} for complexity analysis, and \PilotPsBasic{} / \PilotPsInt{} / \PilotPsAdv{} for problem solving. Contrary to the literature-anchored monotonic expectation $L3>L2>L1$, the pilot shows no such ordering on these compact HumanEval tasks: automatic scores cluster near the top of the scale and the RCIFENI-O-grounded L3 prompts do not outperform the bare L1 input. Two caveats apply. First, LLM-as-judge raters are known to inflate scores and compress between-level variance relative to calibrated human raters~\cite{Liu2023}, so Kendall's $W$ here is a \emph{floor} estimate of the effect size a human panel is expected to detect, not a substitute for the pre-registered partial $\eta^2\geq0.06$ target. Second, HumanEval tasks are compact single-function problems whose solutions fit in a single short response regardless of prompt structure, which limits the room for prompt engineering to help; richer open-ended learning tasks in Phase~2 are expected to widen level differences.
