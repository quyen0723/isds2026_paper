...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, draw, rounded corners=3pt, minimum width=2.1cm, minimum height=0.85cm, align=center, font=\small},
  theo/.style={rectangle, draw, dashed, rounded corners=3pt, minimum width=2.0cm, minimum height=0.7cm, align=center, font=\footnotesize, fill=gray!10},
  arr/.style={->, thick, >=Stealth}
]
\node[box] (lit) {AI\\Literacy};
\node[box, right=of lit] (pq) {Prompt\\Quality (CLEAR)};
\node[box, right=of pq] (mech) {Cognitive\\Mechanisms};
\node[box, right=of mech] (llm) {LLM\\(GPT-4o / Claude)};
\node[box, right=of llm] (out) {AIOQ-R\\5 dims};
...
\draw[->, dashed, gray] (clear) -- (pq);
\draw[->, dashed, gray] (clt) -- (mech);
\draw[->, dashed, gray] (tam) -- (out);
...
\begin{tikzpicture}[
  ...
  cell/.style={rectangle, draw, minimum width=0.9cm, minimum height=0.55cm, align=center},
  phase/.style={rectangle, draw, rounded corners=2pt, minimum width=2.8cm, minimum height=0.7cm, align=center, font=\small}
]
\node[font=\small\bfseries] at (-3.2, 3.4) {(a) Latin Square $4\times4$};
\foreach \i/\row in {1/{T1,T2,T3,T4}, 2/{T2,T3,T4,T1}, 3/{T3,T4,T1,T2}, 4/{T4,T1,T2,T3}} {
  \node at (-6.2, {2.6-\i*0.6}) {G\i};
  \foreach \j/\v [count=\jj from 0] in \row {
    \node[cell, fill=blue!\the\numexpr10+\jj*10\relax] at ({-5.3+\jj*0.95}, {2.6-\i*0.6}) {\v};
  }
}

\node[font=\small\bfseries] at (3.6, 3.4) {(b) $3\times2\times3 = 18$ sessions/participant};
\foreach \x/\lbl in {0/Basic, 1.3/Inter, 2.6/Adv} {
  \node[cell, fill=blue!30] at (\x+1.5, 2.5) {\lbl};
}
\foreach \x/\lbl in {0/GPT-4o, 1.9/Claude} {
  \node[cell, fill=orange!30, minimum width=1.3cm] at (\x+1.9, 1.6) {\lbl};
}
\foreach \x/\lbl in {0/Algo, 1.3/Debug, 2.6/Cplx} {
  \node[cell, fill=green!30] at (\x+1.5, 0.7) {\lbl};
}
\node at (3.0, 0.0) {{\footnotesize $3 \times 2 \times 3 = 18$}};

\node[phase, fill=yellow!15] at (-4.5, -1.4) {Phase~1\\Wk~1--2: Baseline};
\node[phase, fill=yellow!30] at (0.0, -1.4) {Phase~2\\Wk~6--9: Experiment};
\node[phase, fill=yellow!50] at (4.5, -1.4) {Phase~3\\Wk~9--10: Interviews};
\draw[->, thick] (-3.0, -1.4) -- (-1.5, -1.4);
\draw[->, thick] (1.5, -1.4) -- (3.0, -1.4);
\end{tikzpicture}
